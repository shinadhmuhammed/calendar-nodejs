...
class TaskService{
    ...
    public async createTask(title:string,description:string,date:Date,assignedTo:string,createdBy:string){
        try {
    ...
    public async getTask(userId:string){
        try {
            const tasks = await Task.find({ createdBy: userId }).populate('assignedTo', 'username'); 
            return tasks;
        } catch (error:any) {
    ...
    public async getTasksAssignedToUser(userId: string) {
        try {
          const tasks = await Task.find({ assignedTo: userId }).populate('createdBy', 'username');
          return tasks;
        } catch (error:any) {
          console.error('Error fetching tasks:', error.message);
          throw new Error(error.message || 'Error fetching tasks');
      ...
      public async getUsersRole(userId:string){
        try {
            ...
            throw new Error(error.message || 'Error fetching role of the currentuser');
        }
      }
