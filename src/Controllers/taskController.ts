...
class TaskController{
    public async createTask(req:AuthenticatedRequest,res:Response){
        try {
            ...
            const task=await taskService.createTask(title,description,date,assignedTo,createdBy)
    ...
    public async getTasksForCurrentUser(req: AuthenticatedRequest, res: Response) {
        try {
          const userId = req.userId; 
          if (!userId) {
      ...
      public async editTask(req:Request,res:Response){
        try {
            ...
            const updateTask=await taskService.updateTasks(title,description,taskId)
